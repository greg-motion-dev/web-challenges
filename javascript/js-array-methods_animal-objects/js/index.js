...
animals.findIndex(animal => animal.name.length > 5);
...
const allAnimalsInEuropeWeighLessThanOnehundred = animals
  .filter(animal => animal.continents.includes("Europe") && animal.weight < 100);
...
const africanAnimals = animals.filter(animal => animal.continents.includes("Africa"))
const totalWeight = africanAnimals
  .map(animal => animal.weight)
  .reduce((a, b) => (a+b), 0);

const averageWeightOfAllAnimalsInAfrica = totalWeight / africanAnimals.length
